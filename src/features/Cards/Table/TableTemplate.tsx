...
export const TableTemplate = React.memo(() => {
    ...
    const initialState = [
        ...
        {name: 'Pickchi Name', cards: 18, lastUpdated: '18.03.2021', createdBy: 'Stas', userId: 'friend'},
        {name: 'Name Packs', cards: 0, lastUpdated: '19.03.2021', createdBy: 'Vlad', userId: 'main'},
        {name: 'Pickchi Name', cards: 11, lastUpdated: '19.03.2021', createdBy: 'Stas', userId: 'friend'},
        {name: 'Pickchi Packs', cards: 9, lastUpdated: '22.03.2021', createdBy: 'Vlad', userId: 'main'},
        {name: 'Pack Name', cards: 98, lastUpdated: '25.03.2021', createdBy: 'Stas', userId: 'friend'},
    ...
    const cardsFilterDebounce = useCallback(
        debounce((num: number[]) => {
            const newArr = rows.filter(el => cards.filter(el => el >= num[0] && el <= num[1]).includes(el.cards))
            setRows(newArr)
        }, 500), [])
            ...
            <div className={s.header}>
                ...
                <div className={s.packRender}>
                    ...
                    <div className={s.btnGroup}>
                        <button className={btnColor ? s.btnOn : s.btnOff} onClick={filterByUser}>My</button>
                        <button className={btnColor ? s.btnOff : s.btnOn} onClick={filterByAll}>All</button>
                    </div>
                </div>
                <div className={s.cardsRender}>
                    <div className={s.title}>Number of cards</div>
                    <div className={s.range}>
                        <TextField sx={{maxWidth: '70px'}}
                                   type='number'
                                   value={value[0]}
                                   onChange={setRangeOne}
                                   InputProps={{
                                       inputProps: {
                                           max: cards[cards.length - 1], min: cards[0]
                                       }
                                   }}/>
                ...
                <Table aria-label="simple table">
                    <TableHead>
                        <TableRow className={s.mainRow}>
                            <TableCell className={s.cell} align='center'>Name</TableCell>
                            <TableCell className={s.cell} align='center'>Cards</TableCell>
                            <TableCell className={s.cell} align='center'>Last Updated</TableCell>
                            <TableCell className={s.cell} align='center'>Created By</TableCell>
                            <TableCell className={s.cell} align='center'>Actions</TableCell>
                        </TableRow>
                    </TableHead>
                    <TableBody>
                        {(rowsPerPage > 0
